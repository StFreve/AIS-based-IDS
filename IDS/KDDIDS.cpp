...
KDDIDS::TwoStepMutex::TwoStepMutex()
	: counter_(0)
{
}

void KDDIDS::TwoStepMutex::lite_lock()
{
	std::lock_guard<std::mutex> lock(counter_mutex_);
	std::lock_guard<std::mutex> full_lock(full_mutex_);
	++counter_;
}

void KDDIDS::TwoStepMutex::lite_unlock()
{
	std::lock_guard<std::mutex> lock(counter_mutex_);
	std::lock_guard<std::mutex> full_lock(full_mutex_);
	--counter_;
...
void KDDIDS::TwoStepMutex::unlock()
{
	full_mutex_.unlock();
}

KDDIDS::LiteLock::LiteLock(TwoStepMutex& mutex)
	:mutex_(mutex)
{
	mutex_.lite_lock();
}
KDDIDS::LiteLock::~LiteLock()
{
	mutex_.lite_unlock();
}
}